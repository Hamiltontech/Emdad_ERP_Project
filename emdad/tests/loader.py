...
from .suite import emdadSuite
from .result import emdadTestResult


def get_test_modules(module):
    """ Return a list of module for the addons potentially containing tests to
    feed unittest.TestLoader.loadTestsFromModule() """
    results = _get_tests_modules(importlib.util.find_spec(f'emdad.addons.{module}'))
...
def _get_upgrade_test_modules(module):
    upgrade_modules = (
        f"emdad.upgrade.{module}",
        f"emdad.addons.{module}.migrations",
        f"emdad.addons.{module}.upgrades",
    )
...
def make_suite(module_names, position='at_install'):
    ...
    return emdadSuite(sorted(tests, key=lambda t: t.test_sequence))
