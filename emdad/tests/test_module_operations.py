...
import emdad
from emdad.tools import config, topological_sort, unique
from emdad.netsvc import init_logger
from emdad.tests import standalone_tests
import emdad.tests.loader

_logger = logging.getLogger('emdad.tests.test_module_operations')
...
def install(db_name, module_id, module_name):
    with emdad.registry(db_name).cursor() as cr:
...
def parse_args():
    parser = argparse.ArgumentParser(
        description="Script for testing the install / uninstall / reinstall"
                    " cycle of emdad modules. Prefer the 'cycle' subcommand to"
    ...
    parser.add_argument("--addons-path", "-p", type=str, action=CheckAddons,
        help="Comma-separated list of paths to directories containing extra emdad modules")
...
def test_cycle(args):
    ...
    with emdad.registry(args.database).cursor() as cr:
        env = emdad.api.Environment(cr, emdad.SUPERUSER_ID, {})
...
def test_standalone(args):
    ...
    registry = emdad.registry(args.database)
    for module_name in registry._init_modules:
        # import tests for loaded modules
        emdad.tests.loader.get_test_modules(module_name)
...
if __name__ == '__main__':
    ...
    # handle paths option
    if args.addons_path:
        emdad.tools.config['addons_path'] = ','.join([args.addons_path, emdad.tools.config['addons_path']])
        if args.data_dir:
            emdad.tools.config['data_dir'] = args.data_dir
    ...
    logging.config.dictConfig({
        ...
        'loggers': {
            'emdad.modules.loading': {'level': 'CRITICAL'},
            'emdad.sql_db': {'level': 'CRITICAL'},
            'emdad.models.unlink': {'level': 'WARNING'},
            'emdad.addons.base.models.ir_model': {'level': "WARNING"},
        }
    })
