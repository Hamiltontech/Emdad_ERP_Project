# -*- coding: utf-8 -*-
"""
The module :mod:`emdad.tests.common` provides unittest test cases and a few
...
import emdad
from emdad import api
from emdad.models import BaseModel
from emdad.exceptions import AccessError
from emdad.modules.registry import Registry
from emdad.service import security
from emdad.sql_db import BaseCursor, Cursor
from emdad.tools import float_compare, single_email_re, profiler, lower_logging, SQL
from emdad.tools.misc import find_in_path, mute_logger
...
def __getattr__(name):
    ...
    warnings.warn(
        "Since 17.0: emdad.tests.common.Form is deprecated, use emdad.tests.Form",
...
# The emdad library is supposed already configured.
ADDONS_PATH = emdad.tools.config['addons_path']
HOST = '127.0.0.1'
# Useless constant, tests are aware of the content of demo data
ADMIN_USER_ID = emdad.SUPERUSER_ID
...
def standalone(*tags):
    ...
    ``tags`` and the corresponding emdad module name.
    """
    def register(func):
        # register func by emdad module name
        if func.__module__.startswith('emdad.addons.'):
...
def new_test_user(env, login='', groups='base.group_user', context=None, **kwargs):
     ...
     * email: it is either the login (if it is a valid email) or a generated
       ...
       to email being required for most emdad operations;
    """
    if not login:
...
class MetaCase(type):
    ...
    def __init__(cls, name, bases, attrs):
        ...
        if cls.__module__.startswith('emdad.addons.'):
...
class BaseCase(case.TestCase, metaclass=MetaCase):
    """ Subclass of TestCase for emdad-specific code. This class is abstract and
    expects self.registry, self.cr and self.uid to be initialized by subclasses.
    """

    longMessage = True      # more verbose error message by default: https://www.emdad.com/r/Vmh
    ...
    def run(self, result):
        ...
        if getattr(testMethod, '_retry', True) and getattr(self, '_retry', True):
            tests_run_count = int(os.environ.get('emdad_TEST_FAILURE_RETRIES', 0)) + 1
        else:
            tests_run_count = 1
    ...
    def browse_ref(self, xid):
        ...
        :param xid: fully-qualified :term:`external identifier`, in the form
                    :samp:`{module}.{identifier}`
        :raise: ValueError if not found
        :returns: :class:`~emdad.models.BaseModel`
        """
    ...
    def debug_mode(self):
        ...
        with patch('emdad.models.BaseModel.user_has_groups', user_has_groups):
            yield
    ...
    def assertQueries(self, expected, flush=True):
        ...
        with patch('emdad.sql_db.Cursor.execute', execute):
            with patch('emdad.osv.expression.get_unaccent_wrapper', get_unaccent_wrapper):
        ...
        if self.warm:
            ...
            with patch('random.random', lambda: 1):
                ...
                if count != expected:
                    ...
                    if "/emdad/addons/" in filename:
                        filename = filename.rsplit("/emdad/addons/", 1)[1]
                    if count > expected:
...
class TransactionCase(BaseCase):
    ...
    cr: Cursor = None
    muted_registry_logger = mute_logger(emdad.modules.registry._logger.name)
    ...
    def _gc_filestore(cls):
        ...
        with emdad.registry(get_db_name()).cursor() as cr:
            gc_env = api.Environment(cr, emdad.SUPERUSER_ID, {})
    ...
    def setUpClass(cls):
        ...
        cls.registry = emdad.registry(get_db_name())
        ...
        cls.env = api.Environment(cls.cr, emdad.SUPERUSER_ID, {})
...
def save_test_file(test_name, content, prefix, extension='png', logger=_logger, document_type='Screenshot'):
    ...
    screenshots_dir = pathlib.Path(emdad.tools.config['screenshots']) / get_db_name() / 'screenshots'
...
class ChromeBrowser:
    ...
    def __init__(self, test_class, headless=True):
        ...
        self.user_data_dir = tempfile.mkdtemp(suffix='_chrome_emdad')

        otc = emdad.tools.config
...
which leads to stray network requests and inconsistencies."""
    ...
    def _from_remoteobject(self, arg):
        ...
        # of the way emdad widgets are created they all appear as Class(...)
...
class HttpCase(TransactionCase):
    ...
    def setUpClass(cls):
        ...
        cls.xmlrpc_url = f'http://{HOST}:{emdad.tools.config["http_port"]:d}/xmlrpc/2/'
    ...
    def _wait_remaining_requests(self, timeout=10):
        ...
        def get_http_request_threads():
            return [t for t in threading.enumerate() if t.name.startswith('emdad.service.http.request.')]
        ...
        if request_threads:
            ...
            emdad.tools.misc.dumpstacks()

    def logout(self, keep_db=True):
        self.session.logout(keep_db=keep_db)
        emdad.http.root.session_store.save(self.session)

    def authenticate(self, user, password, browser: ChromeBrowser = None):
        if getattr(self, 'session', None):
            emdad.http.root.session_store.delete(self.session)

        self.session = session = emdad.http.root.session_store.new()
        session.update(emdad.http.get_default_session(), db=get_db_name())
        session.context['lang'] = emdad.http.DEFAULT_LANG
        ...
        emdad.http.root.session_store.save(session)
    ...
    @classmethod
    def base_url(cls):
        return f"http://{HOST}:{emdad.tools.config['http_port']}"
    ...
    def start_tour(self, url_path, tour_name, step_delay=None, **kwargs):
        ...
        code = kwargs.pop('code', "emdad.startTour('%s', %s)" % (tour_name, json.dumps(options)))
        ready = kwargs.pop('ready', "emdad.isTourReady('%s')" % tour_name)
    ...
    def profile(self, **kwargs):
        ...
        return profiler.Nested(_profiler, patch('emdad.http.Request._get_profiler_context_manager', route_profiler))
...
def tagged(*tags):
    ...
    By default, all Test classes from emdad.tests.common have a test_tags
