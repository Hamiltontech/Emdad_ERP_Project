...
import emdad
import emdad.tools as tools
import emdad.release as release
from emdad.tools import pycompat
from emdad.tools.misc import file_path
...
class UpgradeHook(object):
    """Makes the legacy `migrations` package being `emdad.upgrade`"""

    def find_spec(self, fullname, path=None, target=None):
        if re.match(r"^emdad\.addons\.base\.maintenance\.migrations\b", fullname):
    ...
    def load_module(self, name):
        ...
        canonical_upgrade = name.replace("emdad.addons.base.maintenance.migrations", "emdad.upgrade")
...
def initialize_sys_path():
    """
    Setup the addons path ``emdad.addons.__path__`` with various defaults
    and explicit directories.
    """
    # hook emdad.addons on data dir
    dd = os.path.normcase(tools.config.addons_data_dir)
    if os.access(dd, os.R_OK) and dd not in emdad.addons.__path__:
        emdad.addons.__path__.append(dd)

    # hook emdad.addons on addons paths
    for ad in tools.config['addons_path'].split(','):
        ad = os.path.normcase(os.path.abspath(tools.ustr(ad.strip())))
        if ad not in emdad.addons.__path__:
            emdad.addons.__path__.append(ad)

    # hook emdad.addons on base module path
    base_path = os.path.normcase(os.path.abspath(os.path.join(os.path.dirname(os.path.dirname(__file__)), 'addons')))
    if base_path not in emdad.addons.__path__ and os.path.isdir(base_path):
        emdad.addons.__path__.append(base_path)

    # hook emdad.upgrade on upgrade-path
    from emdad import upgrade
    ...
    # create decrecated module alias from emdad.addons.base.maintenance.migrations to emdad.upgrade
    spec = importlib.machinery.ModuleSpec("emdad.addons.base.maintenance", None, is_package=True)
    maintenance_pkg = importlib.util.module_from_spec(spec)
    maintenance_pkg.migrations = upgrade
    sys.modules["emdad.addons.base.maintenance"] = maintenance_pkg
    sys.modules["emdad.addons.base.maintenance.migrations"] = upgrade

    # hook deprecated module alias from openerp to emdad and "crm"-like to emdad.addons
...
def get_module_path(module, downloaded=False, display_warning=True):
    ...
    if re.search(r"[\/\\]", module):
        return False
    for adp in emdad.addons.__path__:
...
def get_module_filetree(module, dir='.'):
    ...
    files = emdad.tools.osutil.listdir(path, True)
...
def get_resource_from_path(path):
    ...
    """
    resource = False
    sorted_paths = sorted(emdad.addons.__path__, key=len, reverse=True)
...
def load_openerp_module(module_name):
    ...
    qualname = f'emdad.addons.{module_name}'
...
def get_modules():
    ...
    plist = []
    for ad in emdad.addons.__path__:
