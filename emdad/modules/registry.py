...
import emdad
from emdad.modules.db import FunctionStatus
from emdad.osv.expression import get_unaccent_wrapper
from .. import SUPERUSER_ID
from emdad.sql_db import TestCursor
from emdad.tools import (
    config, existing_tables, lazy_classproperty,
    lazy_property, sql, Collector, OrderedSet, SQL,
    format_frame
)
from emdad.tools.func import locked
from emdad.tools.lru import LRU

_logger = logging.getLogger(__name__)
_schema = logging.getLogger('emdad.schema')
...
class Registry(Mapping):
    ...
    def new(cls, db_name, force_demo=False, status=None, update_module=False):
        ...
        try:
            ...
            try:
                emdad.modules.load_modules(registry, force_demo, status, update_module)
            except Exception:
                emdad.modules.reset_modules_state(db_name)
                raise
        except Exception:
    ...
    def init(self, db_name):
        ...
        self._assertion_report = emdad.tests.result.emdadTestResult()
        ...
        self._db = emdad.sql_db.db_connect(db_name)
        ...
        with closing(self.cursor()) as cr:
            self.has_unaccent = emdad.modules.db.has_unaccent(cr)
            self.has_trigram = emdad.modules.db.has_trigram(cr)
    ...
    def init_models(self, cr, model_names, context, install=True):
        ...
        env = emdad.api.Environment(cr, SUPERUSER_ID, context)
