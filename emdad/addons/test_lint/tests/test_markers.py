...
class TestConflictMarkers(lint_case.LintCase):
    ...
    def test_conflict_markers(self):
        """ Test that there are no conflict markers left in emdad files """

        counter = 0

        emdad_path = os.path.abspath(os.path.dirname(emdad.__file__))
        paths = emdad.addons.__path__ + [emdad_path]
        paths.remove(os.path.join(emdad_path, 'addons'))  # avoid checking emdad/addons twice
