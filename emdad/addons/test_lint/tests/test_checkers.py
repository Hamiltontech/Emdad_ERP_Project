...
from . import _emdad_checker_sql_injection
...
class TestSqlLint(TransactionCase):
    def check(self, testtext):
        ...
        result = run(
            [pylint_bin,
             f'--rcfile={os.devnull}',
             '--load-plugins=_emdad_checker_sql_injection',
             '--disable=all',
    ...
    def test_sql_injection_detection(self):
        ...
        checker = _emdad_checker_sql_injection.emdadBaseChecker(self.linter)

        node = _emdad_checker_sql_injection.astroid.extract_node("""
        def test(): 
            arg = "test"
            arg = arg + arg
