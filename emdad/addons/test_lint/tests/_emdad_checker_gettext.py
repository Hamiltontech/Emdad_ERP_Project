...
def parse_version(s):
    # can't use emdad.tools.parse_version because pythonpath is screwed from
    # inside pylint on runbot
    return [s.rjust(3, '0') for s in s.split('.')]

class emdadBaseChecker(BaseChecker):
    ...
    msgs = {
        'E8502': (
            ...
            'gettext-variable',
            'See https://www.emdad.com/documentation/17.0/developer/misc/i18n/translations.html#variables'
        )
    }
...
def register(linter):
    linter.register_checker(emdadBaseChecker(linter))
