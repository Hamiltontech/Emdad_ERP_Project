...
import emdad
from emdad import SUPERUSER_ID, Command
from emdad.exceptions import AccessError
from emdad.tests import TransactionCase
from emdad.tools.misc import mute_logger
...
class TestIRRuleFeedback(Feedback):
    ...
    def debug_mode(self):
        emdad.http._request_stack.push(Mock(db=self.env.cr.dbname, env=self.env, debug=True))
        self.addCleanup(emdad.http._request_stack.pop)
        self.env.flush_all()
...
Note: this might be a multi-company issue. Switching company may help - in emdad, not in real life!
