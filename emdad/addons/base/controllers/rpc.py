...
import emdad
from emdad.http import Controller, route, dispatch_rpc, request, Response
from emdad.fields import Date, Datetime, Command
from emdad.tools import lazy, ustr
from emdad.tools.misc import frozendict
...
# User code must use the exceptions defined in ``emdad.exceptions`` (not
...
def xmlrpc_handle_exception_int(e):
    if isinstance(e, emdad.exceptions.RedirectWarning):
        fault = xmlrpc.client.Fault(RPC_FAULT_CODE_WARNING, str(e))
    elif isinstance(e, emdad.exceptions.AccessError):
        fault = xmlrpc.client.Fault(RPC_FAULT_CODE_ACCESS_ERROR, str(e))
    elif isinstance(e, emdad.exceptions.AccessDenied):
        fault = xmlrpc.client.Fault(RPC_FAULT_CODE_ACCESS_DENIED, str(e))
    elif isinstance(e, emdad.exceptions.UserError):
        fault = xmlrpc.client.Fault(RPC_FAULT_CODE_WARNING, str(e))
    else:
        info = sys.exc_info()
...
def xmlrpc_handle_exception_string(e):
    ...
    elif isinstance(e, emdad.exceptions.MissingError):
    ...
    #InternalError
        ...
        info = sys.exc_info()
        formatted_info = "".join(traceback.format_exception(*info))
        fault = xmlrpc.client.Fault(emdad.tools.exception_to_unicode(e), formatted_info)

    return xmlrpc.client.dumps(fault, allow_none=None, encoding=None)


class emdadMarshaller(xmlrpc.client.Marshaller):
    ...
    # Historically, emdad is sending binary as base64 string.
...
xmlrpc.client.Marshaller = emdadMarshaller
