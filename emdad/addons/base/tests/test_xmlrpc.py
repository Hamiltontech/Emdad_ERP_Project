...
from emdad.http import _request_stack

import emdad
import emdad.tools
from emdad.tests import common
from emdad.service import common as auth, model
...
class TestXMLRPC(common.HttpCase):
    ...
    def test_xmlrpc_defaultdict_marshalling(self):
        ...
        self.patch(self.registry['res.users'], 'context_get',
                   emdad.api.model(lambda *_: collections.defaultdict(int)))
    ...
    def _json_call(self, *args):
        self.opener.post("http://%s:%s/jsonrpc" % (common.HOST, emdad.tools.config['http_port']), json={
            'jsonrpc': '2.0',
            'id': None,
            'method': 'call',
            'params': {
