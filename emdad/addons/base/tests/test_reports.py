...
@emdad.tests.tagged('post_install', '-at_install', 'post_install_l10n')
class TestReports(emdad.tests.TransactionCase):
...
class TestReportsRenderingCommon(emdad.tests.HttpCase):
    ...
    def save_pdf(self):
        ...
        emdad.tests.save_test_file(self._testMethodName, self.last_pdf_content, 'pdf_', 'pdf', document_type='Report PDF', logger=_logger)
...
@emdad.tests.tagged('post_install', '-at_install', 'pdf_rendering')
...
@emdad.tests.tagged('post_install', '-at_install', '-standard', 'pdf_rendering')
