...
from emdad.addons.base.tests import test_mail_examples
from emdad.addons.base.tests.common import MockSmtplibCase
from emdad.tests import tagged, users
...
class TestIrMailServer(TransactionCase, MockSmtplibCase):
    ...
    def test_mail_server_config_bin(self):
        """ Test the configuration provided in the emdad-bin arguments. This config
        ...
        emdad-bin argument "--from-filter".
        """
        IrMailServer = self.env['ir.mail_server']

        # Remove all mail server so we will use the emdad-bin arguments
        ...
        # for from_filter in ICP, overwrite the one from emdad-bin
    ...
    def test_mail_server_config_cli(self):
        ...
        "cli". It should take the configuration from the emdad-bin argument. The
        ...
        for mail_from, (expected_smtp_from, expected_msg_from, expected_mail_server) in zip(
            [
                # check that the CLI server take the configuration in the emdad-bin argument
