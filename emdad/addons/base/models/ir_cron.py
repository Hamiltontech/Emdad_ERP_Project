...
BASE_VERSION = emdad.modules.get_manifest('base')['version']
MAX_FAIL_TIME = timedelta(hours=5)  # chosen with a fair roll of the dice

# custom function to call instead of default PostgreSQL's `pg_notify`
emdad_NOTIFY_FUNCTION = os.getenv('emdad_NOTIFY_FUNCTION', 'pg_notify')
...
class ir_cron(models.Model):
    ...
    def create(self, vals_list):
        for vals in vals_list:
            vals['usage'] = 'ir_cron'
        if os.getenv('emdad_NOTIFY_CRON_CHANGES'):
        ...
        try:
            ...
            with db.cursor() as cron_cr:
                ...
                for job_id in (job['id'] for job in jobs):
                    ...
                    registry = emdad.registry(db_name)
    ...
    def _check_modules_state(cls, cr, jobs):
        ...
        # reset_module_states.
        emdad.modules.reset_modules_state(cr.dbname)
    ...
    def _callback(self, cron_name, server_action_id, job_id):
        ...
        try:
            ...
            emdad.netsvc.log(_logger, logging.DEBUG, 'cron.object.execute', (self._cr.dbname, self._uid, '*', cron_name, server_action_id), depth=log_depth)
            start_time = False
    ...
    def write(self, vals):
        self._try_lock()
        if ('nextcall' in vals or vals.get('active')) and os.getenv('emdad_NOTIFY_CRON_CHANGES'):
            self._cr.postcommit.add(self._notifydb)
    ...
    def _trigger_list(self, at_list):
        ...
        if min(at_list) <= now or os.getenv('emdad_NOTIFY_CRON_CHANGES'):
            self._cr.postcommit.add(self._notifydb)
        return triggers

    def _notifydb(self):
        """ Wake up the cron workers
        The emdad_NOTIFY_CRON_CHANGES environment variable allows to force the notifydb on both
        ir_cron modification and on trigger creation (regardless of call_at)
        """
        with emdad.sql_db.db_connect('postgres').cursor() as cr:
            query = sql.SQL("SELECT {}('cron_trigger', %s)").format(sql.Identifier(emdad_NOTIFY_FUNCTION))
