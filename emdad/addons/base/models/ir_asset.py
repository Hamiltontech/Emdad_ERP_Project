...
import emdad
import emdad.modules.module  # get_manifest, don't from-import it
from emdad import api, fields, models, tools
from emdad.tools import misc
from emdad.tools.constants import ASSET_EXTENSIONS, EXTERNAL_ASSET
...
class IrAsset(models.Model):
    ...
    def _fill_asset_paths(self, bundle, asset_paths, seen, addons, installed, **assets_params):
        ...
        for addon in addons:
            for command in emdad.modules.module._get_manifest_cached(addon)['assets'].get(bundle, ()):
    ...
    def _topological_sort(self, addons_tuple):
        ...
        def mapper(addon):
            manif = emdad.modules.module._get_manifest_cached(addon)
    ...
    def _get_installed_addons_list(self):
        ...
        return self.env.registry._init_modules.union(emdad.conf.server_wide_modules or [])
    ...
    def _get_paths(self, path_def, installed):
        ...
        Ex: ('/base/static/file.js', '/home/user/source/emdad/emdad/addons/base/static/file.js', 643636800)
        ...
        addon = path_parts[0]
        addon_manifest = emdad.modules.module._get_manifest_cached(addon)
