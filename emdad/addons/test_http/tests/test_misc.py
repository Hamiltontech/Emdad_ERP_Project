...
import emdad
from emdad.http import root, content_disposition
from emdad.tests import tagged
from emdad.tests.common import HOST, new_test_user, get_db_name, BaseCase
from emdad.tools import config, file_path
from emdad.addons.test_http.controllers import CT_JSON

from emdad.addons.test_http.utils import TEST_IP
...
class TestHttpMisc(TestHttpBase):
    ...
    def test_misc2_local_redirect(self):
        def local_redirect(path):
            fake_req = emdad.tools.misc.DotDict(db=False)
            return emdad.http.Request.redirect(fake_req, path, local=True).headers['Location']
    ...
    def test_misc3_is_static_file(self):
        ...
        self.assertEqual(root.get_static_file(f'emdad.com/{uri}', host='emdad.com'), path, "Valid file with valid host")
        self.assertEqual(root.get_static_file(f'http://emdad.com/{uri}', host='emdad.com'), path, "Valid file with valid host")
        ...
        self.assertIsNone(root.get_static_file(f'emdad.com/{uri}'), "No host allowed")
        self.assertIsNone(root.get_static_file(f'http://emdad.com/{uri}'), "No host allowed")
    ...
    def test_misc5_geoip(self):
        ...
        headers = {
            ...
            'X-Forwarded-Host': 'emdad.com',
            'X-Forwarded-Proto': 'https'
        }
        with patch.dict('emdad.tools.config.options', {'proxy_mode': True}):
    ...
    def test_misc6_upload_file_retry(self):
        from emdad.addons.test_http import controllers  # pylint: disable=C0415
...
class TestHttpEnsureDb(TestHttpBase):
    ...
    def test_ensure_db1_grant_db(self):
        ...
        self.assertEqual(emdad.http.root.session_store.get(res.cookies['session_id']).db, 'db0')
    ...
    def test_ensure_db3_change_db(self):
        ...
        new_session = emdad.http.root.session_store.get(res.cookies['session_id'])
