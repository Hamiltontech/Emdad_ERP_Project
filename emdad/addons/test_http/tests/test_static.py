...
import emdad
from emdad.tests import new_test_user, tagged, RecordCapturer
from emdad.tools import config, file_open, image_process
...
class TestHttpStaticLogo(TestHttpStaticCommon):
    ...
    def setUpClass(cls):
        ...
        super_user = cls.env['res.users'].browse([emdad.SUPERUSER_ID])
    ...
    def test_upload_small_file(self):
        ...
        with RecordCapturer(self.env['ir.attachment'], []) as capture, \
            ...
            res = self.opener.post(
                ...
                files={'ufile': file},
                data={
                    'csrf_token': emdad.http.Request.csrf_token(self),
