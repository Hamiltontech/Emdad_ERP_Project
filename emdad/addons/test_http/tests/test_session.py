...
import emdad
from emdad.tests.common import get_db_name
...
class TestHttpSession(TestHttpBase):
    ...
    @mute_logger('emdad.http')  # greeting_none called ignoring args {'debug'}
    ...
    def test_session1_default_session(self):
        ...
        with patch.object(emdad.http.root.session_store, 'save') as mock_save:
    ...
    def test_session3_logout_15_0_geoip(self):
        ...
        with self.assertLogs('emdad.http', level='WARNING') as (_, warnings):
            res = self.multidb_url_open('/test_http/ensure_db', dblist=['db1', 'db2'])

        self.assertEqual(warnings, [
            "WARNING:emdad.http:Logged into database 'idontexist', but dbfilter rejects it; logging session out.",
        ])
    ...
    def test_session7_serializable(self):
        ...
        # Values forbidden by emdad, raising a warning
        for value in [
            str,
            int,
            float,
            ...
            with self.assertLogs(level="WARNING"):
                ...
                self.assertFalse(emdad.http.root.session_store.session_class({'foo': value}, 1234).foo)
