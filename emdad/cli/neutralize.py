...
class Neutralize(Command):
    ...
    def run(self, args):
        parser = emdad.tools.config.parser
        ...
        opt = emdad.tools.config.parse_config(args)

        dbname = emdad.tools.config['db_name']
        if not dbname:
        ...
        try:
            with emdad.sql_db.db_connect(dbname).cursor() as cursor:
                if opt.to_stdout:
                    installed_modules = emdad.modules.neutralize.get_installed_modules(cursor)
                    queries = emdad.modules.neutralize.get_neutralization_queries(installed_modules)
                    ...
                    emdad.modules.neutralize.neutralize_database(cursor)
