...
__author__ = emdad.release.author
__version__ = emdad.release.version

# Also use the `emdad` logger for the main script.
_logger = logging.getLogger('emdad')

re._MAXCACHE = 4096  # default is 512, a little too small for emdad
...
def report_configuration():
    ...
    """
    config = emdad.tools.config
    _logger.info("emdad version %s", __version__)
    if os.path.isfile(config.rcfile):
        _logger.info("Using configuration file at " + config.rcfile)
    _logger.info('addons paths: %s', emdad.addons.__path__)
...
def setup_pid_file():
    ...
    """
    config = emdad.tools.config
    if not emdad.evented and config['pidfile']:
        pid = os.getpid()
...
def export_translation():
    ...
    with open(config["translate_out"], "wb") as buf:
        registry = emdad.modules.registry.Registry.new(dbname)
        with registry.cursor() as cr:
            emdad.tools.trans_export(config["language"],
...
def import_translation():
    ...
    registry = emdad.modules.registry.Registry.new(dbname)
    with registry.cursor() as cr:
        translation_importer = emdad.tools.translate.TranslationImporter(cr)
        translation_importer.load_file(config["translate_in"], config["language"])
        translation_importer.save(overwrite=overwrite)

def main(args):
    check_root_user()
    emdad.tools.config.parse_config(args)
    check_postgres_user()
    ...
    if config['db_name']:
        ...
        for db_name in preload:
            try:
                emdad.service.db._create_empty_database(db_name)
                ...
                if err.pgcode == errorcodes.INSUFFICIENT_PRIVILEGE:
                    ...
                    raise err
            except emdad.service.db.DatabaseExists:
                pass
    ...
    if config['workers']:
        emdad.multi_process = True

    stop = config["stop_after_init"]

    setup_pid_file()
    rc = emdad.service.server.start(preload=preload, stop=stop)
    sys.exit(rc)

class Server(Command):
    """Start the emdad server (default command)"""
    def run(self, args):
        emdad.tools.config.parser.prog = f'{Path(sys.argv[0]).name} {self.name}'
        main(args)
