...
import emdad
from emdad.exceptions import UserError, ValidationError
from emdad.http import request
from emdad.models import check_method_name
from emdad.tools import DotDict
from emdad.tools.translate import _, translate_sql_constraint
...
def dispatch(method, params):
    ...
    registry = emdad.registry(db).check_signaling()
...
def execute_cr(cr, uid, obj, method, *args, **kw):
    ...
    result = retrying(partial(emdad.api.call_kw, recs, method, args, kw), env)
...
def execute(db, uid, obj, method, *args, **kw):
    with emdad.registry(db).cursor() as cr:
...
def retrying(func, env):
    ...
    :param emdad.api.Environment env: The environment where the registry
