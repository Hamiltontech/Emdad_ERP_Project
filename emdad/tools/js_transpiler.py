"""
This code is what let us use ES6-style modules in emdad.
Classic emdad modules are composed of a top-level :samp:`emdad.define({name},{dependencies},{body_function})` call.
This processor will take files starting with an `@emdad-module` annotation (in a comment) and convert them to classic modules.
If any file has the ``/** emdad-module */`` on top of it, it will get processed by this class.
It performs several operations to get from ES6 syntax to the usual emdad one with minimal changes.
...
from emdad.tools.misc import OrderedSet

_logger = logging.getLogger(__name__)

def transpile_javascript(url, content):
    """
    Transpile the code from native JS modules to custom emdad modules.
    ...
    legacy_emdad_define = get_aliased_emdad_define_content(module_path, content)
    ...
    steps = [
        ...
        partial(wrap_with_emdad_define, module_path, dependencies),
    ]
    for s in steps:
        content = s(content)
    if legacy_emdad_define:
        content += legacy_emdad_define
    return content
...
def url_to_module_path(url):
    """
    emdad modules each have a name. (emdad.define("<the name>", [<dependencies>], function (require) {...});
...
def wrap_with_emdad_define(module_path, dependencies, content):
    """
    Wraps the current content (source code) with the emdad.define call.
    ...
    """
    return f"""emdad.define({module_path!r}, {list(dependencies)}, function (require) {{
'use strict';
let __exports = {{}};
{content}
return __exports;
...
emdad_MODULE_RE = re.compile(r"""
    \s*                                       # some starting space
    \/(\*|\/).*\s*                            # // or /*
    @emdad-module                              # @emdad-module
...
def is_emdad_module(content):
    """
    Detect if the file is a native emdad module.
    We look for a comment containing @emdad-module.

    :param content: source code
    :return: is this a emdad module that need transpilation ?
    """
    result = emdad_MODULE_RE.match(content)
    return bool(result)


def get_aliased_emdad_define_content(module_path, content):
    """
    ...
    Example:
    If we have a require call somewhere in the emdad source base being:
    ...
    > emdad.define("web/chrome/abstract_action", ['web.AbstractAction'], function (require) {
    ...
    .. code-block:: javascript
        ...
        /** @emdad-module */
        // after
        /** @emdad-module alias=web.AbstractAction */
    ...
    .. code-block:: javascript
        ...
        /** @emdad-module */
        // after
        /** @emdad-module alias=web.AbstractAction default=false */

    :return: the alias content to append to the source code.
    """
    matchobj = emdad_MODULE_RE.match(content)
    if matchobj:
        alias = matchobj['alias']
        if alias:
            if matchobj['default']:
                return """\nemdad.define(`%s`, ['%s'], function (require) {
                        return require('%s');
