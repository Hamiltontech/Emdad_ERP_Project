...
import emdad
from emdad.tools.config import config
...
class Cloc(object):
    ...
    def count_path(self, path, exclude=None):
        path = path.rstrip('/')
        exclude_list = []
        for i in emdad.modules.module.MANIFEST_NAMES:
    ...
    def count_modules(self, env):
        ...
        exclude_heuristic = [emdad.modules.get_module_path(m, display_warning=False) for m in STANDARD_MODULES]
        ...
        for module_name in module_list:
            module_path = os.path.realpath(emdad.modules.get_module_path(module_name))
            if module_path:
    ...
    def count_customization(self, env):
        ...
        # the module should be emdad/studio unless it comes from an imported module install
    ...
    def count_database(self, database):
        registry = emdad.registry(config['db_name'])
        with registry.cursor() as cr:
            uid = emdad.SUPERUSER_ID
    ...
    def report(self, verbose=False, width=None):
        ...
        # Render
        s = fmt.format(k="emdad cloc", lines="Line", other="Other", code="Code")
        s += hr
