...
from emdad.loglevels import get_encodings, ustr, exception_to_unicode     # noqa
...
def exec_pg_environ():
    ...
    """
    env = os.environ.copy()
    if emdad.tools.config['db_host']:
        env['PGHOST'] = emdad.tools.config['db_host']
    if emdad.tools.config['db_port']:
        env['PGPORT'] = str(emdad.tools.config['db_port'])
    if emdad.tools.config['db_user']:
        env['PGUSER'] = emdad.tools.config['db_user']
    if emdad.tools.config['db_password']:
        env['PGPASSWORD'] = emdad.tools.config['db_password']
    return env
...
def file_path(file_path, filter_ext=('',), env=None):
    ...
    addons_paths = emdad.addons.__path__ + [root_path]
...
def file_open(name, mode="r", filter_ext=None, env=None):
    ...
    Examples::
        ...
        >>> with file_open('/opt/emdad/addons/hr/static/description/icon.png', 'rb') as f:
        ...     contents = f.read()
...
def file_open_temporary_directory(env):
    ...
    Examples::
        ...
        >>> with emdad.tools.file_open_temporary_directory(self.env) as module_dir:
        ...    with zipfile.ZipFile('foo.zip', 'r') as z:
        ...        z.extract('foo/__manifest__.py', module_dir)
        ...    with emdad.tools.file_open('foo/__manifest__.py', env=self.env) as f:
        ...        manifest = f.read()
...
class mute_logger(logging.Handler):
    ...
    Can be used as context manager or decorator::
        ...
        @mute_logger('emdad.plic.ploc')
        def do_stuff():
            blahblah()

        with mute_logger('emdad.foo.bar'):
            do_suff()
    """
...
def ignore(*exc):
    warnings.warn("Since 16.0 `emdad.tools.ignore` is replaced by `contextlib.suppress`.", DeprecationWarning, stacklevel=2)
...
def format_date(env, value, lang_code=False, date_format=False):
    ...
    if isinstance(value, str):
        ...
        if len(value) > DATE_LENGTH:
            ...
            value = emdad.fields.Datetime.from_string(value)
            value = emdad.fields.Datetime.context_timestamp(env['res.lang'], value)
        else:
            value = emdad.fields.Datetime.from_string(value)
...
def format_datetime(env, value, tz=False, dt_format='medium', lang_code=False):
    ...
    if not value:
        return ''
    if isinstance(value, str):
        timestamp = emdad.fields.Datetime.from_string(value)
    else:
        timestamp = value
