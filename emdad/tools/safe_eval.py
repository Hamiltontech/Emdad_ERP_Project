...
def safe_eval(expr, globals_dict=None, locals_dict=None, mode="eval", nocopy=False, locals_builtins=False, filename=None):
    ...
    except emdad.exceptions.UserError:
        raise
    except emdad.exceptions.RedirectWarning:
        raise
...
`emdad.tools.safe_eval.wrap_module` to generate a wrapper recursively
whitelisting allowed attributes.

Pre-wrapped modules are provided as attributes of `emdad.tools.safe_eval`.
""")
    return d
