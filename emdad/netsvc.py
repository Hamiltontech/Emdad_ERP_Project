...
class PostgreSQLHandler(logging.Handler):
    ...
    def emit(self, record):
        ...
        if not dbname:
            return
        with contextlib.suppress(Exception), tools.mute_logger('emdad.sql_db'), sql_db.db_connect(dbname, allow_uri=True).cursor() as cr:
...
def init_logger():
    ...
    if sys.version_info[:2] == (3, 9):
        ...
        warnings.filterwarnings('ignore', r'^Sampling from a set', category=DeprecationWarning, module='emdad')
    ...
    warnings.filterwarnings('ignore', category=BytesWarning, module='emdad.tools.image')
    ...
    if os.name == 'posix' and isinstance(handler, logging.StreamHandler) and (is_a_tty(handler.stream) or os.environ.get("emdad_PY_COLORS")):
...
DEFAULT_LOG_CONFIGURATION = [
    'emdad.http.rpc.request:INFO',
    'emdad.http.rpc.response:INFO',
    ':INFO',
]
PSEUDOCONFIG_MAPPER = {
    'debug_rpc_answer': ['emdad:DEBUG', 'emdad.sql_db:INFO', 'emdad.http.rpc:DEBUG'],
    'debug_rpc': ['emdad:DEBUG', 'emdad.sql_db:INFO', 'emdad.http.rpc.request:DEBUG'],
    'debug': ['emdad:DEBUG', 'emdad.sql_db:INFO'],
    'debug_sql': ['emdad.sql_db:DEBUG'],
    'info': [],
    'runbot': ['emdad:RUNBOT', 'werkzeug:WARNING'],
    'warn': ['emdad:WARNING', 'werkzeug:WARNING'],
    'error': ['emdad:ERROR', 'werkzeug:ERROR'],
    'critical': ['emdad:CRITICAL', 'werkzeug:CRITICAL'],
}
