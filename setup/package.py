...
version_info = ...
nt_service_name = ...
exec(open(os.path.join(ROOTDIR, 'emdad', 'release.py'), 'rb').read())
...
DOCKERUSER = """
RUN mkdir /var/lib/emdad && \
    groupadd -g %(group_id)s emdad && \
    useradd -u %(user_id)s -g emdad emdad -d /var/lib/emdad && \
    mkdir /data && \
    chown emdad:emdad /var/lib/emdad /data
USER emdad
""" % {'group_id': os.getgid(), 'user_id': os.getuid()}


class emdadTestTimeoutError(Exception):
    pass


class emdadTestError(Exception):
    pass
...
def _rpc_count_modules(addr='http://127.0.0.1', port=8069, dbname='mycompany'):
    ...
    if len(modules) > 1:
        ...
        if toinstallmodules:
            ...
            raise emdadTestError("Installation of package failed")
        else:
        ...
        raise emdadTestError("Package test: FAILED. Not able to install base.")
...
def publish(args, pub_type, extensions):
    ...
    for extension in extensions:
        release = glob("%s/emdad_*.%s" % (args.build_dir, extension))
        if release:
...
def _prepare_build_dir(args, win32=False, move_addons=True):
    ...
    run_cmd(cmd + ['%s/' % args.emdad_dir, args.build_dir])
    if not move_addons:
        return
    for addon_path in glob(os.path.join(args.build_dir, 'addons/*')):
        if args.blacklist is None or os.path.basename(addon_path) not in args.blacklist:
            try:
                shutil.move(addon_path, os.path.join(args.build_dir, 'emdad/addons'))
...
class Docker():
    ...
    def __init__(self, args):
        ...
        """
        self.args = args
        self.tag = 'emdad-%s-%s-nightly-tests' % (DOCKERVERSION, self.arch)
    ...
    def run(self, cmd, build_dir, container_name, user='emdad', exposed_port=None, detach=False, timeout=None):
    ...
    def test_emdad(self):
        logging.info('Starting to test emdad install test')
        start_time = time.time()
        while self.is_running() and (time.time() - start_time) < INSTALL_TIMEOUT:
            time.sleep(5)
            if os.path.exists(os.path.join(args.build_dir, 'emdad.pid')):
                try:
        ...
        if self.is_running():
            self.stop()
            raise emdadTestTimeoutError('emdad pid file never appeared after %s sec' % INSTALL_TIMEOUT)
        raise emdadTestError('Error while installing/starting emdad after %s sec.\nSee testlogs.txt in build dir' % int(time.time() - start_time))
...
class DockerTgz(Docker):
    ...
    def build(self):
        ...
        self.run('python3 setup.py sdist --quiet --formats=gztar,zip', self.args.build_dir, 'emdad-src-build-%s' % TSTAMP)
        os.rename(glob('%s/dist/emdad-*.tar.gz' % self.args.build_dir)[0], '%s/emdad_%s.%s.tar.gz' % (self.args.build_dir, VERSION, TSTAMP))
        os.rename(glob('%s/dist/emdad-*.zip' % self.args.build_dir)[0], '%s/emdad_%s.%s.zip' % (self.args.build_dir, VERSION, TSTAMP))
    ...
    def start_test(self):
        ...
        cmds = [
            ...
            'su postgres -s /bin/bash -c "createuser -s emdad"',
            'su emdad -s /bin/bash -c "python3 -m venv /var/lib/emdad/emdadvenv"',
            'su emdad -s /bin/bash -c "/var/lib/emdad/emdadvenv/bin/python3 -m pip install --upgrade pip"',
            'su emdad -s /bin/bash -c "/var/lib/emdad/emdadvenv/bin/python3 -m pip install -r /opt/release/requirements.txt"',
            f'su emdad -s /bin/bash -c "/var/lib/emdad/emdadvenv/bin/python3 -m pip install /data/src/emdad_{VERSION}.{TSTAMP}.tar.gz"',
            'su emdad -s /bin/bash -c "createdb mycompany"',
            'su emdad -s /bin/bash -c "/var/lib/emdad/emdadvenv/bin/emdad -d mycompany -i base --stop-after-init"',
            'su emdad -s /bin/bash -c "/var/lib/emdad/emdadvenv/bin/emdad -d mycompany --pidfile=/data/src/emdad.pid"',
        ]
        self.run(' && '.join(cmds), self.args.build_dir, 'emdad-src-test-%s' % TSTAMP, user='root', detach=True, exposed_port=8069, timeout=300)
        self.test_emdad()
...
class DockerDeb(Docker):
    ...
    def build(self):
        ...
        cmds = ["sed -i '1s/^.*$/emdad (%s.%s) stable; urgency=low/' debian/changelog" % (VERSION, TSTAMP)]
        ...
        cmds.append('mv ../emdad_* ./')
        self.run(' && '.join(cmds), self.args.build_dir, 'emdad-deb-build-%s' % TSTAMP)
    ...
    def start_test(self):
        ...
        cmds = [
            ...
            f'/usr/bin/apt-get install -y /data/src/emdad_{VERSION}.{TSTAMP}_all.deb',
            'su emdad -s /bin/bash -c "emdad -d mycompany -i base --pidfile=/data/src/emdad.pid"',
        ]
        self.run(' && '.join(cmds), self.args.build_dir, 'emdad-deb-test-%s' % TSTAMP, user='root', detach=True, exposed_port=8069, timeout=300)
        self.test_emdad()
...
class DockerRpm(Docker):
    ...
    def build(self):
        ...
        rpmbuild_dir = '/var/lib/emdad/rpmbuild'
        cmds = [
            'cd /data/src',
            'mkdir -p dist',
            'rpmdev-setuptree -d',
            f'cp -a /data/src/setup/rpm/emdad.spec {rpmbuild_dir}/SPECS/',
            f'tar --transform "s/^\\./emdad-{VERSION}/" -c -z -f {rpmbuild_dir}/SOURCES/emdad-{VERSION}.tar.gz .',
            f'rpmbuild -bb --define="%version {VERSION}" /data/src/setup/rpm/emdad.spec',
            f'mv {rpmbuild_dir}/RPMS/noarch/emdad*.rpm /data/src/dist/'
        ]
        self.run(' && '.join(cmds), self.args.build_dir, f'emdad-rpm-build-{TSTAMP}')
        os.rename(glob('%s/dist/emdad-*.noarch.rpm' % self.args.build_dir)[0], '%s/emdad_%s.%s.rpm' % (self.args.build_dir, VERSION, TSTAMP))
    ...
    def start_test(self):
        ...
        cmds = [
            ...
            'sleep 5',
            'su postgres -c "createuser -s emdad"',
            'su emdad -c "createdb mycompany"',
            'dnf install -d 0 -e 0 /data/src/emdad_%s.%s.rpm -y' % (VERSION, TSTAMP),
            'su emdad -s /bin/bash -c "emdad -c /etc/emdad/emdad.conf -d mycompany -i base --stop-after-init"',
            'su emdad -s /bin/bash -c "emdad -c /etc/emdad/emdad.conf -d mycompany --pidfile=/data/src/emdad.pid"',
        ]
        self.run(' && '.join(cmds), args.build_dir, 'emdad-rpm-test-%s' % TSTAMP, user='root', detach=True, exposed_port=8069, timeout=300)
        self.test_emdad()
    ...
    def gen_rpm_repo(self, args, rpm_filepath):
        ...
        self.run('createrepo /data/src/', temp_path, 'emdad-rpm-createrepo-%s' % TSTAMP)
...
class KVMWinBuildExe(KVM):
    def run(self):
        ...
        remote_build_dir = '/cygdrive/c/emdadbuild/server/'

        self.ssh("mkdir -p build")
        logging.info("Syncing emdad files to virtual machine...")
...
class KVMWinTestExe(KVM):
    def run(self):
        ...
        setup_path = glob("%s/emdad_setup_*.exe" % self.args.build_dir)[0]
        setupfile = setup_path.split('/')[-1]
        setupversion = setupfile.split('emdad_setup_')[1].split('.exe')[0]

        self.rsync(['%s' % setup_path, '%s@127.0.0.1:' % self.login])
        self.ssh("TEMP=/tmp ./%s /S" % setupfile)
        self.ssh('PGPASSWORD=openpgpwd /cygdrive/c/"Program Files"/"emdad %s"/PostgreSQL/bin/createdb.exe -e -U openpg mycompany' % setupversion)
        self.ssh('netsh advfirewall set publicprofile state off')
        self.ssh('/cygdrive/c/"Program Files"/"emdad {sv}"/python/python.exe \'c:\\Program Files\\emdad {sv}\\server\\emdad-bin\' -d mycompany -i base --stop-after-init'.format(sv=setupversion))
...
def parse_args():
    ...
    # Windows VM
    ap.add_argument("--vm-winxp-image", default='/home/emdad/vm/win1036/win10_winpy36.qcow2', help="%(default)s")
    ap.add_argument("--vm-winxp-ssh-key", default='/home/emdad/vm/win1036/id_rsa', help="%(default)s")
