...
!ifndef SERVICENAME
	!define SERVICENAME 'emdad-server-${VERSION}'
!endif

!ifndef TOOLSDIR
	!define TOOLSDIR 'c:\emdadbuild'
!endif

!define PRODUCT_NAME "emdad"
...
!define UNINSTALL_REGISTRY_KEY_SERVER "${UNINSTALL_BASE_REGISTRY_KEY}\emdad Server ${VERSION}"
...
OutFile "emdad_setup_${VERSION}.exe"
...
!define MUI_ICON "${PIXMAPS_PATH}\emdad-icon.ico"

!define MUI_WELCOMEFINISHPAGE_BITMAP "${PIXMAPS_PATH}\emdad-intro.bmp"
!define MUI_UNWELCOMEFINISHPAGE_BITMAP "${PIXMAPS_PATH}\emdad-intro.bmp"
!define MUI_HEADERIMAGE
!define MUI_HEADERIMAGE_BITMAP "${PIXMAPS_PATH}\emdad-slogan.bmp"
...
!define MUI_FINISHPAGE_LINK_LOCATION "https://www.emdad.com/page/contactus"
...
; English
LangString DESC_emdad_Server ${LANG_ENGLISH} "Install the emdad Server with all the emdad standard modules."
LangString DESC_PostgreSQL ${LANG_ENGLISH} "Install the PostgreSQL RDBMS used by emdad."
LangString DESC_FinishPage_Link ${LANG_ENGLISH} "Contact emdad for Partnership and/or Support"
LangString DESC_AtLeastOneComponent ${LANG_ENGLISH} "You have to choose at least one component"
LangString DESC_CanNotInstallPostgreSQL ${LANG_ENGLISH} "You can not install the PostgreSQL database without the emdad Server"
...
LangString Profile_AllInOne ${LANG_ENGLISH} "emdad Server And PostgreSQL Server"
LangString Profile_Server ${LANG_ENGLISH} "emdad Server Only"
LangString Profile_IOT ${LANG_ENGLISH} "emdad IoT"
LangString TITLE_emdad_Server ${LANG_ENGLISH} "emdad Server"
LangString TITLE_PostgreSQL ${LANG_ENGLISH} "PostgreSQL Database"
LangString TITLE_IOT ${LANG_ENGLISH} "emdad IoT"
LangString TITLE_Nginx ${LANG_ENGLISH} "Nginx WebServer"
LangString TITLE_Ghostscript ${LANG_ENGLISH} "Ghostscript interpreter"
LangString DESC_FinishPageText ${LANG_ENGLISH} "Start emdad"

; French
LangString DESC_emdad_Server ${LANG_FRENCH} "Installation du Serveur emdad avec tous les modules emdad standards."
LangString DESC_PostgreSQL ${LANG_FRENCH} "Installation de la base de données PostgreSQL utilisée par emdad."
LangString DESC_FinishPage_Link ${LANG_FRENCH} "Contactez emdad pour un Partenariat et/ou du Support"
LangString DESC_AtLeastOneComponent ${LANG_FRENCH} "Vous devez choisir au moins un composant"
LangString DESC_CanNotInstallPostgreSQL ${LANG_FRENCH} "Vous ne pouvez pas installer la base de données PostgreSQL sans le serveur emdad"
...
LangString Profile_AllInOne ${LANG_FRENCH} "Serveur emdad Et Serveur PostgreSQL"
LangString Profile_Server ${LANG_FRENCH} "Seulement Le Serveur emdad"
LangString Profile_IOT ${LANG_FRENCH} "emdad IoT"
LangString TITLE_emdad_Server ${LANG_FRENCH} "Serveur emdad"
LangString TITLE_PostgreSQL ${LANG_FRENCH} "Installation du serveur de base de données PostgreSQL"
LangString TITLE_IOT ${LANG_FRENCH} "emdad IoT"
...
LangString DESC_FinishPageText ${LANG_FRENCH} "Démarrer emdad"
...
Section $(TITLE_emdad_Server) Sectionemdad_Server
    SectionIn 1 2 3
    ...
    # If there is a previous install of the emdad Server, keep the login/password from the config file
    WriteIniStr "$INSTDIR\server\emdad.conf" "options" "db_host" $TextPostgreSQLHostname
    WriteIniStr "$INSTDIR\server\emdad.conf" "options" "db_user" $TextPostgreSQLUsername
    WriteIniStr "$INSTDIR\server\emdad.conf" "options" "db_password" $TextPostgreSQLPassword
    WriteIniStr "$INSTDIR\server\emdad.conf" "options" "db_port" $TextPostgreSQLPort
    # Fix the addons path
    WriteIniStr "$INSTDIR\server\emdad.conf" "options" "addons_path" "$INSTDIR\server\emdad\addons"
    WriteIniStr "$INSTDIR\server\emdad.conf" "options" "bin_path" "$INSTDIR\thirdparty"
    # Set data_dir
    WriteIniStr "$INSTDIR\server\emdad.conf" "options" "data_dir" "$INSTDIR\sessions"
    ...
    ${If} $HasPostgreSQL == 0
        WriteIniStr "$INSTDIR\server\emdad.conf" "options" "pg_path" "$INSTDIR\PostgreSQL\bin"
    ${EndIf}

    # Productivity Apps
    WriteIniStr "$INSTDIR\server\emdad.conf" "options" "default_productivity_apps" "True"
    DetailPrint "Installing Windows service"
    nsExec::ExecTOLog '"$INSTDIR\python\python.exe" "$INSTDIR\server\emdad-bin" --stop-after-init -c "$INSTDIR\server\emdad.conf" --logfile "$INSTDIR\server\emdad.log" -s'
    nsExec::ExecToLog '"$INSTDIR\nssm\win64\nssm.exe" install ${SERVICENAME} "$INSTDIR\python\python.exe"'
    nsExec::ExecToLog '"$INSTDIR\nssm\win64\nssm.exe" set ${SERVICENAME} AppDirectory "$\"$INSTDIR\python$\""'
    nsExec::ExecToLog '"$INSTDIR\nssm\win64\nssm.exe" set ${SERVICENAME} AppParameters "\"$INSTDIR\server\emdad-bin\" -c "\"$INSTDIR\server\emdad.conf\"'
    nsExec::ExecToLog '"$INSTDIR\nssm\win64\nssm.exe" set ${SERVICENAME} ObjectName "LOCALSERVICE"'
    AccessControl::GrantOnFile  "$INSTDIR" "LOCALSERVICE" "FullAccess"

    Call RestartemdadService
SectionEnd
...
Section $(TITLE_PostgreSQL) SectionPostgreSQL
    ...
    ExecWait '"$TEMP\$postgresql_exe_filename" \
        ...
        --servicename "PostgreSQL_For_emdad" \
...
Section $(TITLE_IOT) IOT
    ...
    WriteIniStr "$INSTDIR\server\emdad.conf" "options" "server_wide_modules" "web,hw_posbox_homepage,hw_drivers"
    WriteIniStr "$INSTDIR\server\emdad.conf" "options" "list_db" "False"
    WriteIniStr "$INSTDIR\server\emdad.conf" "options" "max_cron_threads" "0"
    nsExec::ExecToStack '"$INSTDIR\python\python.exe" "$INSTDIR\server\emdad-bin" genproxytoken'
    pop $0
    pop $ProxyTokenPwd
    Call RestartemdadService
SectionEnd
...
Section $(TITLE_Nginx) Nginx
    ...
    File "..\..\emdad\addons\point_of_sale\tools\posbox\overwrite_after_init\etc\ssl\certs\nginx-cert.crt"
    File "..\..\emdad\addons\point_of_sale\tools\posbox\overwrite_after_init\etc\ssl\private\nginx-cert.key"
SectionEnd
...
Section -Post
    ...
    WriteRegStr HKLM       "${UNINSTALL_REGISTRY_KEY}" "HelpLink" "support@emdad.com"
    WriteRegStr HKLM       "${UNINSTALL_REGISTRY_KEY}" "HelpTelephone" "+32.81.81.37.00"
    WriteRegStr HKLM       "${UNINSTALL_REGISTRY_KEY}" "URLInfoAbout" "https://www.emdad.com"
    WriteRegStr HKLM       "${UNINSTALL_REGISTRY_KEY}" "Contact" "sales@emdad.com"
...
!insertmacro MUI_FUNCTION_DESCRIPTION_BEGIN
    !insertmacro MUI_DESCRIPTION_TEXT ${Sectionemdad_Server} $(DESC_emdad_Server)
...
Function .onInit
    ...
    ${If} $previous_install_dir == ""
        StrCpy $INSTDIR "$PROGRAMFILES64\emdad ${VERSION}"
...
Function ShowProxyTokenDialogPage
    ...
    doProxyToken:
        ...
        ${NSD_CreateLabel} 0 0 100% 25% "Here is your access token for the emdad IOT, please write it down in a safe place, you will need it to configure the IOT"
        Pop $ProxyTokenLabel
...
Function ComponentLeave
    SectionGetFlags ${Sectionemdad_Server} $0
...
Function RestartemdadService
    DetailPrint "Restarting emdad Service"
