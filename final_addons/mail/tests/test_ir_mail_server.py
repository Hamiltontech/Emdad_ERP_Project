...
from emdad.tools import config, mute_logger
...
class TestIrMailServer(MailCommon):
    ...
    def test_mail_server_get_test_email_from(self):
        ...
        for (default_from, from_filter), expected_test_email in zip(
            [
                ...
                f'notifications@{self.mail_alias_domain.name}',
                f'emdad@{self.mail_alias_domain.name}',
                self.env.user.email,
                self.env.user.email,
                self.env.user.email,
                self.env.user.email,
    ...
    @mute_logger('emdad.models.unlink', 'emdad.addons.base.models.ir_mail_server')
