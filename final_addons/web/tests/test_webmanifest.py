...
from emdad.tests.common import tagged
...
class WebManifestRoutesTest(HttpCaseWithUserDemo):
    ...
    def test_webmanifest(self):
        ...
        data = response.json()
        self.assertEqual(data["name"], "emdad")
        self.assertEqual(data["scope"], "/web")
        self.assertEqual(data["start_url"], "/web")
        self.assertEqual(data["display"], "standalone")
        self.assertEqual(data["background_color"], "#ef7757")
        self.assertEqual(data["theme_color"], "#ef7757")
        self.assertEqual(data["prefer_related_applications"], False)
        self.assertCountEqual(data["icons"], [
            {'src': '/web/static/img/emdad-icon-192x192.png', 'sizes': '192x192', 'type': 'image/png'},
            {'src': '/web/static/img/emdad-icon-512x512.png', 'sizes': '512x512', 'type': 'image/png'}
        ])
    ...
    def test_apple_touch_icon(self):
        ...
        response = self.url_open("/web/static/img/emdad-icon-ios.png")
        self.assertEqual(response.status_code, 200)

        document = self.url_open("/web")
        self.assertIn(
            '<link rel="apple-touch-icon" href="/web/static/img/emdad-icon-ios.png"/>', document.text,
