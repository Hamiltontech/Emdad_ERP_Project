...
from emdad.tools import ustr, file_open
...
class WebManifest(http.Controller):
    ...
    def webmanifest(self):
        ...
        """
        web_app_name = request.env['ir.config_parameter'].sudo().get_param('web.web_app_name', 'emdad')
        manifest = {
            'name': web_app_name,
            'scope': '/web',
            'start_url': '/web',
            'display': 'standalone',
            'background_color': '#ef7757',
            'theme_color': '#ef7757',
            'prefer_related_applications': False,
        }
        icon_sizes = ['192x192', '512x512']
        manifest['icons'] = [{
            'src': '/web/static/img/emdad-icon-%s.png' % size,
            'sizes': size,
            'type': 'image/png',
    ...
    def _icon_path(self):
        return 'web/static/img/emdad-icon-192x192.png'
    ...
    def offline(self):
        ...
        return request.render('web.webclient_offline', {
            'emdad_icon': base64.b64encode(file_open(self._icon_path(), 'rb').read())
        })
