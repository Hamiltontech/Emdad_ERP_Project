...
import emdad
from emdad.tests.common import HttpCase, HOST
from emdad.tools.misc import hmac, DotDict, frozendict
...
def MockRequest(
    ...
    request = Mock(
        ...
        type='http',
        future_response=emdad.http.FutureResponse(),
        params={},
        redirect=env['ir.http']._redirect,
        session=DotDict(
            emdad.http.get_default_session(),
            geoip={'country_code': country_code},
            sale_order_id=sale_order_id,
            website_sale_current_pl=website_sale_current_pl,
        ),
        geoip=emdad.http.GeoIP('127.0.0.1'),
    ...
    #
    #   router = emdad.http.root.get_db_router()
    ...
    with contextlib.ExitStack() as s:
        emdad.http._request_stack.push(request)
        s.callback(emdad.http._request_stack.pop)
        s.enter_context(patch('emdad.http.root.get_db_router', router))
