...
import * as emdadEditorLib from "@web_editor/js/editor/emdad-editor/src/utils/utils";
import { Component, onMounted, useRef, useState } from "@emdad/owl";
...
const getDeepRange = emdadEditorLib.getDeepRange;
const getTraversedNodes = emdadEditorLib.getTraversedNodes;
...
const wSnippetMenu = weSnippetEditor.SnippetsMenu.extend({
    ...
    _getDragAndDropOptions(options = {}) {
        // TODO: This is currently not in use by emdad's D&D
        // There is currently no way in emdad D&D to offset the edge scrolling.
     ...
     */
    _getSelection() {
        return this.options.wysiwyg.emdadEditor.document.getSelection();
    },
    /**
     * @override
     */
    _addToolbar() {
        ...
        this.options.wysiwyg.emdadEditor.computeFontSizeSelectorValues();
    },
    /**
    ...
    _handleTextOptions(targetEl, optionClassList, applyTextOption = () => {}) {
        ...
        if (selectedTextEl) {
            const restoreCursor = emdadEditorLib.preserveCursor(this.$body[0].ownerDocument);
            ...
            this.options.wysiwyg.emdadEditor.historyResetLatestComputedSelection();
            this._disableTextOptions(targetEl);
            this.options.wysiwyg.emdadEditor.historyStep(true);
            restoreCursor();
        } else {
            ...
            if ($snippet) {
                ...
                this.options.wysiwyg.emdadEditor.historyStep();
            } else {
