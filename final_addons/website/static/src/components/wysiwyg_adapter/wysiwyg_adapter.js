...
import { isMediaElement } from '@web_editor/js/editor/emdad-editor/src/utils/utils';
...
import { Component, onWillStart, useEffect, onWillUnmount } from "@emdad/owl";
...
export class WysiwygAdapterComponent extends Wysiwyg {
    ...
    async startEdition() {
        ...
        const webEditorFilterMutationRecords = this.emdadEditor.options.filterMutationRecords;
        Object.assign(this.emdadEditor.options, {
            /**
             * @override
             */
        ...
        // Disable emdadEditor observer's while setting up classes
        this.emdadEditor.observerUnactive();
        ...
        // the dirty observer (not to be confused with emdadEditor
        // observer) as the widgets might trigger DOM mutations.
        this._setObserver();
        this.emdadEditor.observerActive();
    }
    /**
    ...
    async destroy() {
        ...
        const mod = await emdad.loader.modules.get('@website/snippets/s_social_media/options')[Symbol.for('default')];
        mod.clearDbSocialValuesCache();

        const formOptionsMod = await emdad.loader.modules.get('@website/snippets/s_website_form/options')[Symbol.for('default')];
    ...
    _setObserver() {
        const processRecords = (records) => {
            records = this.emdadEditor.filterMutationRecords(records);
            ...
            this.emdadEditor.automaticStepSkipStack();
        ...
        this.emdadEditor.addEventListener('observerUnactive', () => {
            if (this.observer) {
                processRecords(this.observer.takeRecords());
                this.observer.disconnect();
            }
        });
        this.emdadEditor.addEventListener('observerActive', this._observe.bind(this));
    }
    /**
     * @private
     */
    ...
    _addEditorMessages() {
        ...
        for (let htmlEl of $wrap.not("[placeholder]").filter('[data-oe-sanitize="no_block"]')) {
            ...
            if (htmlEl.innerText.trim().length === 0) {
                ...
                // It is removed by emdadEditor when text is entered.
        ...
        const commands = [
            {
                ...
                fontawesome: 'fa-info',
                isDisabled: () => !this.emdadEditor.isSelectionInBlockRoot(),
                callback: () => {
    ...
    async _createSnippetsMenuInstance(options = {}) {
        const snippetsEditor = await emdad.loader.modules.get('@website/js/editor/snippets.editor')[Symbol.for('default')];
    ...
    _toggleMegaMenu(toggleEl) {
        ...
        this.emdadEditor.observerUnactive("toggleMegaMenu");
        megaMenuEl.classList.add('o_no_parent_editor');
        this.emdadEditor.observerActive("toggleMegaMenu");
