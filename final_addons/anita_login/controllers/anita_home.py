...
from emdad.addons.web.controllers.home import Home, SIGN_UP_REQUEST_PARAMS
from emdad.addons.web.controllers.utils import ensure_db, is_user_internal
...
try:
    ...
    from emdad.tools._vendor.send_file import send_file
...
class AdminHome(Home):
    ...
    def web_login(self, redirect=None, **kw):
        ...
        if not request.uid:
            request.update_env(user=emdad.SUPERUSER_ID)

        values = {k: v for k, v in request.params.items() if k in SIGN_UP_REQUEST_PARAMS}
        try:
            values['databases'] = http.db_list()
        except emdad.exceptions.AccessDenied:
        ...
        if request.httprequest.method == 'POST':
            ...
            except emdad.exceptions.AccessDenied as e:
                if e.args == emdad.exceptions.AccessDenied().args:
